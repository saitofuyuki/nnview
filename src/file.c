...
fi_list_vars( int fileid )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return (nio_fi_list_vars(fileid));
      }
    else
      {
...
fi_long_var_name( int fileid, char *var_name )
{
	...
	return( netcdf_long_var_name( fileid, var_name ));
      ...
      {
        return(nio_long_var_name(fileid, var_name));
      }
    else
		{
		fprintf( stderr, "?unknown file_type passed to fi_long_var_name: %d\n",
			file_type );
		exit( -1 );
		}
}
...
fi_var_units( int fileid, char *var_name )
{
	...
	return( netcdf_var_units( fileid, var_name ));
      ...
      {
        return(nio_var_units(fileid, var_name));
      }
    else
		{
...
fi_dim_calendar( int fileid, char *dim_name )
{
    ...
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return(nio_dim_calendar (fileid, dim_name));
      }
    else
        {
...
fi_dim_units( int fileid, char *dim_name )
{
  if (file_type == FILE_TYPE_NETCDF)
    ...
    {
      return(nio_dim_units(fileid, dim_name));
    }
  else
        {
...
fi_n_dims( int id, char *var_name )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return (nio_fi_n_dims (id, var_name));
      }
    else
        {
...
fi_scannable_dims( int fileid, char *var_name )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return(nio_scannable_dims(fileid, var_name));
      }
    else
        {
...
fi_var_size( int fileid, char *var_name )
{
  if (file_type == FILE_TYPE_NETCDF)
    ...
    {
      return (nio_fi_var_size (fileid, var_name));
    }
  else
    {
...
fi_dim_id_to_name( int fileid, char *var_name, int dim_id )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return(nio_dim_id_to_name(fileid, var_name, dim_id));
      }
    else
        {
...
fi_dim_name_to_id( int fileid, char *var_name, char *dim_name )
{
	...
	return( netcdf_dim_name_to_id( fileid, var_name, dim_name ));
      ...
      {
        return(nio_dim_name_to_id(fileid, var_name, dim_name));
      }
    else
      {
		fprintf( stderr, "?unknown file_type passed to fi_dim_name_to_id: %d\n",
			file_type );
		exit( -1 );
		}
}
...
fi_get_data( NCVar *var, size_t *virt_start_pos, size_t *count, void *data )
{
	...
	if( file_type == FILE_TYPE_NETCDF )
      ...
      {
        nio_fi_get_data(file->id, var->name, act_start_pos,
                        count, data, (NetCDFOptions *)var->first_file->aux_data);
      }
	else
		{
	...
	for( it=virt_start_pos[0]; it<(virt_start_pos[0]+count[0]); it++ ) {
		...
		if( file_type == FILE_TYPE_NETCDF )
        ...
        else if (file_type == FILE_TYPE_NIO)
          {
			...
			{
			fprintf( stderr, "?unknown file_type passed to fi_get_data_iterate: %d\n",
				file_type );
			exit( -1 );
			}
		}
...
fi_dim_longname( int fileid, char *dim_name )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        return(nio_dim_longname(fileid, dim_name));
      }
    else
        {
        fprintf( stderr, "?unknown file_type passed to fi_dim_longname: %d\n",
			file_type );
		exit( -1 );
		}
}
...
/*printf( "mapped, dim=%s loc=%ld  val=%lf\n", var->dim[dim_id]->name, idx_map, *return_val_double );*/
	...
	*(virt_start_pos+dim_id) = virt_place;
    ...
    if( file_type == FILE_TYPE_NETCDF )
      ...
      {
        ret_val = nio_dim_value(file->id, dim_name, actual_place,
                                return_val_double, return_val_char, virt_place,
                                return_has_bounds, return_bounds_min, return_bounds_max);
      }
	else
...
fi_has_dim_values( int fileid, char *dim_name )
{
  if (file_type == FILE_TYPE_NETCDF)
    ...
    {
      return(nio_has_dim_values(fileid, dim_name));
    }
  else
        {
...
determine_file_type( Stringlist *input_files )
{
    ...
    else if (nio_fi_confirm(input_files->string))
      file_type = FILE_TYPE_NIO;
	else
		{
...
fi_fill_aux_data( int id, char *var_name, FDBlist *fdb )
{
    if (file_type == FILE_TYPE_NETCDF)
      ...
      {
        nio_fill_aux_data(id, var_name, fdb);
      }
    else
        {
        fprintf( stderr, "?unknown file_type passed to fi_fill_aux_data: %d\n",
			file_type );
		exit( -1 );
		}
}
...
fi_fill_value( NCVar *var, float *fill_value )
{
    if( file_type == FILE_TYPE_NETCDF )
      ...
      {
        nio_fill_value(var->first_file->id, var->name, fill_value);
      }
	else
		{
...
fi_recdim_id( int fileid )
{
	...
	return( netcdf_fi_recdim_id( fileid ));
      ...
      {
        return(nio_fi_recdim_id(fileid));
      }
    else
      {
        fprintf(stderr, "?unknown file_type passed to fi_recdim_id: %d\n",
                fileid);
        exit(-1);
      }
}
...
char *fi_get_char_att(int fileid, char *var_name, char *att_name)
{
  if (file_type == FILE_TYPE_NETCDF)
    {
      return(netcdf_get_char_att(fileid, var_name, att_name));
    }
  else if (file_type == FILE_TYPE_NIO)
    {
      return(nio_get_char_att(fileid, var_name, att_name));
    }
  else
    {
      fprintf(stderr, "?unknown file_type passed to fi_get_char_att: %d\n",
              file_type);
      exit(-1);
    }
}
...
size_t fi_check_has_grown(char *file, char *var_name, size_t *ref_sizes, int dim_id)
{
  if (file_type == FILE_TYPE_NETCDF)
    {
      return(netcdf_fi_check_has_grown(file, var_name, ref_sizes, dim_id));
    }
  else if (file_type == FILE_TYPE_NIO)
    {
      return(nio_fi_check_has_grown(file, var_name, ref_sizes, dim_id));
    }
  else
    {
