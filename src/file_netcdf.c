...
int netcdf_fi_initialize( char *name )
{
	...
	if( ierr != NC_NOERR ) {
		fprintf( stderr, "netcdf_fi_initialize: can't properly open file %s\n",
			name );
		exit( -1 );
		}
...
void netcdf_fi_list_vars_v4( Stringlist **retval, int fileid )
{
    ...
    if (options.debug) printf("nnview/group: %s\n", groupname);
...
/*****************************************************************************************************
 * Get size of the var in the last file, to check whether it has grown.
 * Return grown size (so 0 if not grown).
 * Originally view_check_new_data() directly calls netcdf_fi_initialize(),
 * which is not desirable for encapsulation.
 */

size_t
netcdf_fi_check_has_grown(char *file, char *var_name, size_t *ref_sizes, int dim_id)
{
  size_t ret_val;
  int t_ncid;
  size_t *t;
  ...
  t_ncid = netcdf_fi_initialize(file);
  t = netcdf_fi_var_size(t_ncid, var_name);
  ierr = nc_close(t_ncid);

  ret_val = t[dim_id] - ref_sizes[dim_id];
