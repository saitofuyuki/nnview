...
view_check_new_data( int unused )
{
    ...
    dt = fi_check_has_grown(view->variable->last_file->filename,
                            view->variable->name, view->variable->last_file->var_size,
                            timelike_index);
...
view_change_transform( int delta )
{
	...
	switch( options.transform ) {
		...
		case TRANSFORM_LOW2: in_set_label( LABEL_TRANSFORM, "1/2 Low"   ); break;
		...
		case TRANSFORM_SYMHI4:  in_set_label (LABEL_TRANSFORM, "4 SymHi"); break;
		default:
